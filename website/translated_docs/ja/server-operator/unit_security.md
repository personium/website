...
Ansible を利用してPersonium Unitを作成した場合、デフォルトで強い権限を持つUnitマスタートークンが有効になっています。本章ではデフォルトからの変更を推奨する設定についてご説明します。
...
Unitマスタートークンは開発やテストの時に使うことを想定したものであり、特殊なケースを除き多くの場合本番運用ではセキュリティ的観点から無効化すべきものです。
しかし、新規作成されたPersonium にはUnit管理に使用するCellやアカウントが存在しません。そのためAnsible 実行前に任意のUnitマスタートークンを設定し、有効化した状態で構築しています。

### Unitマスタートークンの無効化手順

UnitマスタートークンはAPサーバで以下の手順を行い無効化することが可能です。
...
1. 'personium-unit-config.properties' の修正
    ...
    * io.personium.core.masterToken パラメータをコメントアウトする事でUnitマスタートークンを無効化できます。
...
Ansible でPersonium Unitを管理するアカウントとしてデフォルトでunitadmin というUnitユーザーを作成しています。unitadmin は全てのCellやBoxといったリソースに対して追加・変更・削除といった操作が可能です。そのため、Ansible で構築した環境では共通的に使用するアカウント名のため、セキュリティレベルは下がってしまいます。
...
1. 管理者権限としてCellにアクセス可能なUnit管理用のトークン(Unitユーザトークン)を取得します。
    ...
    Unitユーザトークンは、[Ansible で構築したUnitの環境情報](./Confirm_environment_settings.md)で取得したUnit管理アカウント情報を用いて取得します。<br>
